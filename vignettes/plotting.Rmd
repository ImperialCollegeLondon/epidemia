...
3. `plot_infections`: for plotting hte nymber of infections.
...
Calling `plot_rt` with no arguments other than the fitted `epimodel` plots the repdoduction for every group for the entire date range for which the model was fitted. The 50% and 95% credible intervals are plotted.
...
Use the `group` argument to plot a subset of the available group. For example, we can plot the repduction number for just Austria and Germany.

```{r fig.width=9, fig.height=4, fig.align="center"}
plot_rt(fit, group=c("Austria", "Germany"))
```
...
```{r fig.width=9, fig.height=4, fig.align="center"}
plot_rt(fit, dates=c("09-03-20", NA))
```
