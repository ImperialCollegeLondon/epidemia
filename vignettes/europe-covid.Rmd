...
rt <- epirt(
  ...
  prior_covariance = decov(shape = c(1, rep(0.5, 5)),scale=0.25),
...
```{r prior-check}
plot_rt(fm_prior, groups = "United_Kingdom")
```
...
```{r obs-plots, out.width ="100%", fig.height = 6}
...
```{r rt-plots, out.width = "100%", fig.height = 6}
plot_rt(fm)
```
...
In **epidemia**, estimated effect sizes can be plotted using the `plot.epimodel` 
method. This serves a similar purpose to `plot.stanreg` in **rstanarm**, 
provideing an interface to the **bayesplot** package. The models in **epidemia** 
...
```{r effect-plots, fig.height=4, echo = FALSE}
plot(fm, par_models = "R", par_types = "fixed")
```

Figure \@ref(fig:effect-plots) shows a large negative coefficient for the effect 
of lockdown, suggesting that this is on average the most effective intervention. 
...
bayesplot::mcmc_intervals(mat) + ggplot2::ggtitle("Italy")
```
Figure \@ref(plot-total-effects) has relatively narrow intervals for many of the 
...
\@ref(rt-plots) shows that Sweden did enough to reduce $R$ below one. However, 
it did so without a full lockdown. Given the small effect sizes for other NPIs, 
the model must explain Sweden using the country-specific terms. Figure \@ref(interval-plots)
shows estimated seeds, intercepts and effects of the public policy NPI for each 
...
Figure \@ref(interval-plots) shows that the model estimates a large negative 
coefficient for the effect of banning public events in Sweden. This is 
significantly larger then the effects for other policies - which are not reported 
here.

```{r interval-plots, echo=FALSE, warning=FALSE, message=FALSE, out.width = "100%", fig.height = 4}
library(gridExtra)
...
grid.arrange(p_seeds, p_int, p_pub, nrow=1, padding=unit(0), widths = c(1.5,1,1))
```
