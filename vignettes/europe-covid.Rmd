...
```{r counterfactuals, out.width = "100%"}
shift_earlier <- function(x, k) c(x[-(1:k)], rep(1,k))
newdata <- filter(newdata, date < as.Date("2020-05-05"))

newdata <- mutate(newdata,
  lockdown = public_events + social_distancing_encouraged + self_isolating_if_ill + schools_universities,
  lockdown = 1 * (lockdown) > 0
)
