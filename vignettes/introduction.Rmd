...
\begin{equation}
R_t := 2Af\left(\eta_t\right),
(\#eq:rtpooled)
\end{equation}
where $\eta_t$ is a linear predictor decomposable as
$$
\eta_t = X_t\beta + Z_t b + Q \gamma_t.
$$
We now describe each of these components of $\eta_t$ in turn.

* $X_t$ is a $M \times p$ model matrix for $t$^th^ period, and for the $p$ *pooled* coefficients $\beta$,
* $Z_t$ is a $M \times q$ model matrix for $t$^th^ period, and for $q$ group specific effects $b$, and
* $Q$ is a $M \times v$ model matrix, where $v$ is the total number of random walks. $Q$ is binary and invariant over time, and simply maps each population to a subset of the random walks. $\gamma_t$ represents the values of each walk at time $t$.

The parameterization given by \@ref(eq:rtpooled) is very flexible. The user can choose which parameters to pool and which to retain as group-level effects. Moreover, multiple random walks can be specified and shared between populations, or according to some other grouping variable.

The model for the $M$ infection series remains largely the same. The parameter $\tau$ is pooled between all populations. 
