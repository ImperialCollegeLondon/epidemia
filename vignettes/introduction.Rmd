...
```{r plotItaly}
library(plotly)
p1 <- plot_rt(fit, plotly=T)
p2 <- plot_obs(fit, type="deaths", plotly=T)
subplot(p1,p2, margin=0.1)
...
args$rt <- epirt(
  ...
  prior =rstanarm::normal(location=0,scale=.5),
  prior_intercept = rstanarm::normal(location=0,scale=2),
  prior_covariance = rstanarm::decov(scale=0.1)
)
fit <- do.call("epim", args)
```
```{r plotIU,results='hold',fig.align="center"}
p1 <- plot_rt(fit, plotly=T)
p2 <- plot_obs(fit, type = "deaths", plotly=T)
...
The basic idea behind the model is that the observed quantities (daily deaths, incidence rates etc.) are a function of the latent cumulative infections in the population over time, which we denote by $I(t)$. This is assumed to  evolve according to
\begin{equation}
I'(t) =  \left(\frac{S_0-I(t)}{S_0} \right)C_{\lfloor t \rfloor}R_{\lfloor t \rfloor},
(\#eq:ode)
\end{equation}
where 

* $S_0$ is the susceptible population at $t_0$,
* $C_t$ is a measure of the 'total infectiousness' at $t_0 + t$. Each infected individual has a level of infectiousness depending on the date at which they were initially infected. $C_t$ simply aggregates this across all infected individuals. More on this in Section \@ref(infections),
* $R_t$ is the *unadjusted* time-varying reproduction rate, and intuitively measures the intensity of infectious interactions at $t_0 + t$.

$R_t$ is latent, just like the underlying infections. The key idea is that it may intuitively be explained by a number of covariates; for example non-pharmaceutical interventions, or changes in mobility over time. i.e. reduced mobility may imply fewer interactions between pairs of infected and susceptible individuals, which is captured through a reduced reproduction number.

The remainder of Section \@ref(model-overview) discusses each component of the model in greater depth, starting with the model for $R_t$. Section \@ref(infections) considers the infection model, paying particular attention to initial conditions (seeding), and the simulation of $I(t)$ at discrete intervals. Finally in Section \@ref(observations) the observed data is modeled as a function of these latent infections. 

### Time-Varying Reproduction Number 

Recall that $R_t$ is defined as the *unadjusted* time-varying reproduction rate. The term *unadjusted* is used to make explicit that it is not interpreted as the expected number of new infections per infected individual at a given point in time. This would require an adjustment for the size of the susceptible population, which is done within the parentheses in the right-hand-side of \@ref(eq:ode). $R_t$ is modeled as
\begin{equation}
R_t = 2R' f\left(\eta_t\right),
(\#eq:rt)
\end{equation}
where $R'$ is a constant, $f$ is the standard logistic function (inverse logit), and $\eta_t$ is a linear predictor.

When $\eta_0 = 0$, the constant $R'$ has the special interpretation of being the initial reproduction number. In general however, $\eta_t$ will contain an intercept so that $R_0$ can be treated as unknown. Note that \@ref(eq:rt) implies that $R_t$ cannot be greater than $2R'$. Therefore $R'$ must be chosen carefully to ensure a plausible upper-bound for the series.

In the most general case $\eta_t$ can include pooled coefficients, group specific effects, and autocorrelation terms. While this may seem like overkill for a single population, it is extremely useful when we extend to multiple populations (Section \@ref(multiple-populations)). For this reason, we describe this in full generality here. $\eta_t$ can be decomposed as
\begin{equation}
...
* $Z_t$ is a $q$-dimensional vector and for group specific effects $b$, and
...
Recall that the time series $R_t$ is latent and hence unobserved. The quantities we do observe are linked to it through its effect on the underlying infections in the population, which we now describe.

### Infections

The evolution of infected individuals over time is modeled using a discrete renewal process. New infections today are taken to be a function of previous infections, a generation distribution, and the series $R_t$ described above. This is summarized by \@ref(eq:ode).
...
Data is observed from $L$ processes $Y_t^{(1)} \ldots Y_{t}^{(L)}$ with the subscript $t$ ranging between $1$ and $T$. These processes do not need to be fully observed; i.e. data may be missing for a subset of time periods. The typical example of such data would be daily death (as used in @Flaxman2020) or case incidence data. However, you are not limited to this. In general,
...
(\#eq:alpha)
\end{equation}
for given hyperparameters $\mu_l$ and $\sigma_{\alpha_l}$. In future versions of epidemia, these proportions may themselves be modeled to allow for changes over time. Suppose now that there are $M$ non-overlapping populations with differing start dates $t^{(m)}_0$ and epidemic lengths $T_m$.



## Multiple Populations

Extending to multiple populations is done by allowing partial pooling of model parameters. Suppose now that there are $M$ non-overlapping populations with differing start dates $t^{(m)}_0$ and epidemic lengths $T_m$.

As before, start by considering the model for the time-varying reproduction numbers. Fix $t$ and now let
$$
R_t := (R_{t,1}, \ldots R_{t,M}),
$$
where $R_{t,m}$ is the unadjusted reproduction number at time $t$ for population $m$.

$R_t$ follows the same form as in \@ref(eq:rt) with $\eta_t$ now being  
\begin{equation}
...
The model for the $M$ infection processes remains largely the same. The parameter $\tau$ is pooled between all populations. 

For the observation model, we replace  \@ref(eq:alpha) with
$$
\alpha_{l,m} \sim \mathcal{N}_{[0,1]}(\mu_{l,m}, \sigma^2_{\alpha_l}),
$$
so that \@ref(eq:alpha) is replicated for each population. Notice that the hyperparameters $\sigma^2_{\alpha_l}$ are shared at the population level. 

The dispersion parameters $\phi_l$ are pooled, and the distributions $\pi^{(l)}$ are assumed the same for each population.
...
df <- data.frame(Arguments = c("formula", 
                               ...
                               "prior_PD",
                               "sampling_args"),
                 Description = c("Specifies the model \\@ref(eq:etapooled) for the unadjusted time-varying reproduction number. See [glmer](https://www.rdocumentation.org/packages/lme4/versions/1.1-23/topics/glmer) for more details.",
                                 ...
                                 "If `TRUE` then all covariates in \\@ref(eq:etapooled) are centered (shifted to have mean zero). **WARNING**: *the priors are then interpreted as priors on the centered covariates*. Defaults to `FALSE`.",
                                 ...
                                 "Set the constant $R'$, introduced in \\@ref(eq:rt). This will determine the maximum possible unadjusted $R_t$ (to be clear, not neccessarily $R'$). Defaults to 3.28.",
...
#### `formula` Argument
This specifies the model for the *unadjusted* time-varying reproduction number, given by \@ref(eq:etapooled). The LHS takes the form `R(group,date)`. This is just syntactic sugar to make explicit that the model is for the reproduction number for a given population and date.
...
barplot(deaths$lag, names.arg=1:101)
```
...
Prior distributions should be specified for the parameters in \@ref(eq:etapooled), and for additional model parameters. 
...
The priors for $\beta$ (see \@ref(eq:etapooled)) are set using the `prior` and `prior_intercept` arguments. These will only have an effect if the formula specifies covariates and an intercept respectively. These function  similarly to the same arguments in[`stan_lm`](https://mc-stan.org/rstanarm/reference/stan_lm.html). To understand which families can be used please refer to [rstanarm's priors](https://mc-stan.org/rstanarm/reference/priors.html). 
