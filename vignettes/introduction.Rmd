...
$$
with initial condition $I(N_0-1) = I_{N_0-1}$ and where $c_t$ is a weighted summation over previous infections
$$
c_{t} = \sum_{\tau=0}^{t} (I(\tau) - I(\tau-1)) s_{t-\tau}.
$$

Given the value of $I_{t-1}$, the exact solution to the above ODE at period $t$ can be shown to be 
$$
...
Please note that this infection model departs from @Flaxman2020, and has the advantage of ensuring that cumulative infections are always bounded by the population size.
