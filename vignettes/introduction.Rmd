...
Recall that $R_t$ is defined as the *unadjusted* time-varying reproduction rate. The term *unadjusted* is used to make explicit that it is not interpreted as the average number of new infections $I'(t)/I(t)$ at a given point in time. This would require an adjustment for the size of the susceptible population, which is done within the parentheses in the right-hand-side of \@ref(eq:ode). $R_t$ is modeled as
\begin{equation}
