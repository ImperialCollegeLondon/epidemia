...
**epidemia** is an R-package for fitting Bayesian epidemic models. Here we detail these models and give users enough information to start fitting them independently. We hope that users find the interface provided by the package intuitive. 

The document is organized as follows. Section \@ref(example-usage) provides basic examples with minimal description, so that users can quickly get a feel for the package. Section \@ref(model-overview) digs deeper and gives a formal framework for the models within the scope of the package. Understanding this framework is essential for making the most of the flexibility offered by **epidemia**. Section \@ref(implementation-in-r) discusses the details of `epim`; the primary model fitting function in the package. `epim` is designed to be flexible, and therefore has a number of arguments. This can be intimidating, and so Section \@ref(implementation-in-r) describes these arguments in a lot of detail. Finally Section \@ref(examples) highlights some of the more useful features of the package.
...
Table XXX lists the formal arguments to `epim`.
...
df <- data.frame(Arguments = c("formula", 
                               ...
                               "prior_PD",
                               "sampling_args"),
                 Description = c("An R object of class 'formula'. The left hand side must take the form `R(group,date)`, with `group` representing a factor vector giving the group membership (i.e. country, state, age cohort). `date` must ve a vector of Date objects.",
                                 "A dataframe providing the data used in the model specified by 'formula'. Each term in `formula` must have a corresponding column in `data`.",
                                 "A list providing all available observations. Each element of `obs` represents its own type of observation. More on this below",
                                 "A two column dataframe giving the total population of each group. The first column represents the group and the second giving the corresponding population.",
                                 "A numeric vector representing the serial interval of the disease. This should be a simplex vector - i.e. all elements non-negative and summing to one. The $n$\\textsuperscript{th} element gives the probability an individual infects another $n$ days after disease onset.",
                                 "Number of days $N_0$ for which to seed infections.",
                                 "The algorithm used to fit the model. Must be one of 'sampling', 'meanfield' or 'fullrank'.",
                                 ...
                                 "If `TRUE` then the covariates for the regression on $R$ are centered (shifted to have mean zero). Warning: all of the priors are then interpreted as priors on the centered covariates. Defaults to `FALSE`.",
                                 "Prior on $\\beta$, excluding the intercept. Same as in `rstanarm::stan_glm`",
                                 "Prior on the intercept, if it exists in the model. Same as in `rstanarm::stan_glm`",
                                 "Prior on the covariance matrix of $b$, the group-specific parameters. Only used if the formula has at least one term of the form `(.|.)`.",
                 ...
                 type = c("Formula",
                          ...
                          "Boolean",
                          "list"))
knitr::kable(df, caption = "Formal arguments for the epim function")
```

#### `formula` Argument

This argument specifies a model for the *unadjusted* time-varying reproduction number, introduced in Section \@ref(time-varying-reproduction-number). The object passed must have class `formula`. 

As mentioned in Table xxx, the LHS must take the form `R(group,date)`. This notation is to make explicit that we are modelling the reproduction number for a specific population and date.
