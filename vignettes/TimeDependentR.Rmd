...
args$sampling_args <- list(iter=400,control=list(adapt_delta=0.95,max_treedepth=15))
```
...
```{r plotItaly}
library(gridExtra)
grid.arrange(plot_obs(fit, type="deaths"),plot_infections(fit),plot_rt(fit),nrow=1)
```
...
```{r plotsplineUK}
grid.arrange(plot_obs(fit, type="deaths"),plot_rt(fit),plot_infections(fit),nrow=1)
```
