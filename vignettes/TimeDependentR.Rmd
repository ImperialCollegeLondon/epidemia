...
args$sampling_args <- list(iter=600,control=list(adapt_delta=0.98,max_treedepth=15),seed=7988)
args$group_subset <- c("Italy")
args$data$week <- format(args$data$date, "%V")
args$formula <- R(country,date) ~  1+rw(week) 
...
args <- EuropeCovid
args$data$week <- format(args$data$date, "%V")
args$formula <- R(country,date) ~ 1+ rw(week,country)
