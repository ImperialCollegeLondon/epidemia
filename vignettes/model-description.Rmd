...
# Basic Model {#sec:overview}

We now formulate the basic version of the model for one homogeneous population. The same model can be used for multiple regions or groups jointly. Suppose we observe a time series of data $\mathbf{Y} = (Y_1, \ldots Y_n)$ in a single region. This could for example be daily death or case incidence data. $Y_t$ is modeled as deriving from past new infections $i_s$, $s < t$, and some parameter $\alpha_t$ representing the instantaneous *ascertainment rate*. The general model can be expressed as 
\begin{align}
...
\end{equation}
where $\mathbf{R} = (R_1, \ldots, R_n)$ and $\mathbf{\alpha} = (\alpha_1, \ldots, \alpha_n)$. The posterior distribution is then proportional to prior and likelihood, i.e.
\begin{equation}
...
Nonetheless the form of Equation \@ref(eq:obsmean) is flexible enough to model other types of data: 
this discussed further in Section \@ref(sec:obsextended). 
...
The particular form for $\Sigma$, as well as its prior is discussed in more detail in Section XXX. 
...
**epidemia** can apply a transformation to infections to ensure cumulative infections remain bounded by $S_0$. 
