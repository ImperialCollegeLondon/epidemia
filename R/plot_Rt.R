...
plot_obs.epimodel <- function(object, type, group, levels, ...) {
  if (length(group) > 1)
  ...
  p <-  ggplot2::ggplot(qtl, ggplot2::aes(x=dates)) + 
        ggplot2::geom_ribbon(data = qtl, 
                            ggplot2::aes(ymin=low, 
                                          ymax=up, 
                                          fill=tag)) +
        ggplot2::xlab("") +
        ggplot2::ylab(type) +
        ...
        ggplot2::gtitle(group) +
        ggplot2::geom_bar(data = df, 
                          ggplot2::aes(y = obs, fill = "reported"),
                          fill = "coral4", 
                          stat='identity', 
                          alpha=0.5)
...
.get_quantiles(df, levels) {
  ...
  f <- function(x) quantile(x, 0.5 + levels/200)
  qtl$up <- matrix(t(apply(df, 2, FUN=q)), ncol=1, byrow = F)
  return(qtl)
}
