data {
  ...
  int CT_ID; // index of Connecticut
  int NY_ID; // index of New York
  int MA_ID; // index of New York
  int ME_ID;
  int VT_ID;
  int NJ_ID; 
  int RI_ID; 
...
transformed parameters {
    ...
    
    {
      ...
      for (m in 1:M){
        ...
        Rt[,m] = mu[m] * 2 * inv_logit(-X[m] * alpha - X_partial_regional[m] * alpha_region[Region[m]] - X_partial_state[m] * alpha_state[m]);
        }
        ...
        // if(m == CT_ID) 
        //   prediction[N0:(N0+30),m] += prediction[N0:(N0+30),NY_ID] * 0.003 ;
